...
bot.on("message", async (message) => {
  ...
  if (userInfo >= 50) {
    ...
    message.author.send(
      `Parabéns! Você subiu de nível! Para conferir seu "LvL" use o comando \`'userinfo\``
    );
  }
  ...
  if (
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setColor("RANDOM")
      .setDescription(
        `Olá! <@${message.author.id}> Parece que você está com dúvida, meu prefixo nesse servidor é \`!\` e meu comando de ajuda é \`'ajuda\``
      )
      .setTimestamp()
      .setFooter(
        `Menção executada por ${message.author.tag}`,
  ...
  try {
    ...
    if (message.author.id === bot_id && message.channel.id === channel_id) {
      ...
      let bot_name_3 = bot_name_2.replace(
      ...
      if (bot_name_final === bot.user.tag) {
        // Defina a tag do seu bot 'bot.user.tag' || 'client.user.tag'

        voto.send(
          `:thumbsup: \`` +
            separador[1] +
            `\` votou no ${bot.user.tag}! :tada:\nhttps://botsparadiscord.com/bots/${bot.user.id}`,
          {
