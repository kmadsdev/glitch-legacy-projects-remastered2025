...
exports.run = async (bot, message, args) => {
  if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply(":x: | Você precisa da permissão `Expulsar Membros` para executar esse comando!");
    ...
    if (user.id === "873228616245325906") return message.reply(":x: | Você não pode me expulsar!");
if (user.id === "811227262912299028") return message.reply(":x: | Você não pode expulsar meu desenvolvedor!");
